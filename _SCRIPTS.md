...
- `scripts/run-image-processing.js`
...
- `scripts/utils/filename-sanitizer.js`
...
- `scripts/cleanup-orphaned-markdown.js`
  - Detects markdown without corresponding images (orphaned) and can relocate/report.
- `scripts/utils/*`
  - Shared helpers used across image processing & maintenance tasks.

## Planned / Proposed

- Open Graph share image generator (canvas/SSR based) for claims/quotes/memes.
- `pnpm audit-images` alias for a read-only integrity & naming audit (subset of imageProcessing dry-run).
...
---
Regenerate / expand this index when adding new scripts.
