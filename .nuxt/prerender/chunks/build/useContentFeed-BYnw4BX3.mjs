...
function useContentFeed(providedSearchTerm = vueExports.ref(""), providedContentFilters = vueExports.ref({ claims: true, quotes: true, memes: true })) {
  const searchTerm = providedSearchTerm;
  const contentFilters = providedContentFilters;
  const displayedItems = vueExports.ref([]);
  const allItems = vueExports.ref([]);
  ...
  const limit = vueExports.ref(10);
  const contentCollections = {
    claims: vueExports.ref([]),
    quotes: vueExports.ref([]),
    memes: vueExports.ref([])
  };
  const isInitialized = vueExports.ref(false);
  const claimCount = vueExports.ref(0);
  const quoteCount = vueExports.ref(0);
  const memeCount = vueExports.ref(0);
...
//# sourceMappingURL=useContentFeed-BYnw4BX3.mjs.map
