...
<template>
  <client-only>
    <ModalFrame v-if="modalData" :show="show" @close="emit('close')">
      ...
      <div class="flex flex-col w-full max-w-[min(1000px,calc(100vw-4rem))] h-full max-h-[calc(100vh-8rem)]">
        <!-- Image container -->
        <div class="flex flex-1 justify-center items-center min-h-0">
          <img v-if="modalData?.image" :src="modalData?.image" :alt="modalData?.title"
