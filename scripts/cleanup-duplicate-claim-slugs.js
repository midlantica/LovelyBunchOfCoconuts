...
function decidePair(files) {
  ...
  const styles = new Set(enriched.map(e => (e.isDash ? 'dash' : e.isUnderscore ? 'underscore' : 'other')));
...
function formatReport(result) {
  ...
  for (const e of entries) {
    lines.push(`  - ${e.name}  size=${e.size}  frontMatter=${e.hasFrontMatter}` + (e === winner ? '  <-- keep' : ''));
  }
  return lines.join('\n');
}
...
function run() {
  ...
  if (asJson) {
    const json = decisions.map(d => ({
      ...
      losers: d.entries.filter(e => e !== d.winner).map(e => path.basename(e.name)),
      sizes: Object.fromEntries(d.entries.map(e => [e.name, e.size])),
      frontMatter: Object.fromEntries(d.entries.map(e => [e.name, e.hasFrontMatter]))
    }));
    console.log(JSON.stringify({ count: json.length, decisions: json }, null, 2));
  } else {
    console.log('== Duplicate Claim Slug Report ==');
    console.log(`Found ${decisions.length} duplicate groups (dash vs underscore).`);
    if (!decisions.length) {
  ...
  if (apply && decisions.length) {
    ...
    console.log(`\nMoved ${decisions.reduce((a,d)=>a + d.entries.length -1,0)} duplicate files into backup: ${path.relative(ROOT, backupRoot)}`);
  }
