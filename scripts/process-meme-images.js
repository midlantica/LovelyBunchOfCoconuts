...
if (args.length === 0) {
  console.error(
    'Error: Please provide a subdirectory name within public/memes/'
  )
  console.error(
    'Usage: node process-meme-subdirectory.js <subdirectory-name> [--force]'
  )
  process.exit(1)
}
...
fs.writeFileSync(
  LOG_FILE,
  `Starting subdirectory processing at ${new Date().toISOString()}\n`
)
...
function countCumulativeRenamed(logPath) {
  ...
  const logContent = fs.readFileSync(logPath, 'utf8')
  return logContent.split('\n').filter((line) => line.startsWith('Renamed:'))
    .length
}
...
async function processSubdirectory() {
  try {
    ...
    try {
      ...
      log(
        `Error: Subdirectory ${subdirName} does not exist in ${BASE_MEMES_DIR}`
      )
      process.exit(1)
    }
    ...
    try {
      ...
      await import(path.join(__dirname, './utils/imageProcessing.js')).then(
        (mod) => mod.processImages(TARGET_DIR)
      )
    ...
    if (imageFilesBefore.length === imageFilesAfter.length) {
      ...
      log(
        'Warning: Number of image files changed during renaming, cannot reliably count renames.'
      )
      renamedCount = 0
    }
    ...
    log(
      `Check individual log files for details on renamed files and created markdown files.`
    )
    ...
    const mdSummary = parseLogSummary(
      path.join(__dirname, 'markdown-creation-log.txt'),
      {
    ...
    const maxValue = Math.max(
      ...tableRows.map((row) => String(row.value).length)
    )
