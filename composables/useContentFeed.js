...
export function useContentFeed(providedSearchTerm = ref(""), providedContentFilters = ref({claims: true, quotes: true, memes: true})) {
  // Core state
  ...
  const createContentWall = () => {
    ...
    const uniqueItems = {
      claims: [...new Set(filteredCollections.claims.map(c => c._path))]
        .map(path => filteredCollections.claims.find(c => c._path === path)),
      quotes: [...new Set(filteredCollections.quotes.map(q => q._path))]
        .map(path => filteredCollections.quotes.find(q => q._path === path)),
      memes: [...new Set(filteredCollections.memes.map(m => m._path))]
        .map(path => filteredCollections.memes.find(m => m._path === path))
    };
